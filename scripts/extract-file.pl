...
extract-file.pl - extract one or more files/directories from a virtual machine and place them into a zip file.
...
A libvirt virtual machine domain name.  This will be used to locate its first disk image to be opened read-only.
